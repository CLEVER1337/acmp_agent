...
def main():
    ...
    for i in range(1, n):
        dist = indexed_points[i][0] - indexed_points[i-1][0]
        if dist < min_dist:
            min_dist = dist
            left_idx = indexed_points[i-1][1]
            right_idx = indexed_points[i][1]
        elif dist == min_dist:
            current_left = min(indexed_points[i-1][1], indexed_points[i][1])
            current_right = max(indexed_points[i-1][1], indexed_points[i][1])
